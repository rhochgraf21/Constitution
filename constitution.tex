...
% when describing membership types TODO
...
The Intro Process is designed to provide an easy means for Intro Members to meet existing CSH members, learn CSH history, and allow current CSH members to evaluate them for Active Membership.
...
On a member by member basis, this evaluation process determines if each Intro member has successfully completed the Intro Process requirements \ref{Expectations of an Intro Member}.
A Relative Majority vote with a quorum of two-thirds of the Total Number of Possible Votes is required for the eval to be valid.
...
CSH may choose any of the Outcomes for each member.

\asubsubsubsection{Outcomes}
\renewcommand{\theenumi}{\alph{enumi}} % For this section, we want items to use letters
\begin{enumerate}
	\item Intro Members may be offered Active Membership (\ref{Active Membership}) provided they meet the requirements described in \ref{Expectations of an Intro Member}, at the discretion of CSH.
	...
	\item An Intro member may be given a conditional to complete as a means of making up for missing requirements.
	      A conditional may be proposed by any member present at the Intro Eval and, if it is approved by the Evals Director, is then voted on by CSH.
...
Active members are expected to be active participants in the CSH as defined in \ref{Expectations of House Members}.
...
A Secret Immediate Relative Majority vote with a quorum of two-thirds of the Total Number of Possible Votes is required for the eval to be valid.
Neither absentee nor proxy votes are allowed.
...
If the member has previously passed a Membership Eval they will move to Alumni Membership status at the end of the Standard Operating Session (\ref{Alumni Membership Selection}).
...
Alumni Membership is open to all former Active members who passed at least one Membership Eval and departed for reasons other than revocation of CSH Membership.
...
Honorary Membership is open to a person whom CSH feels has contributed great personal effort to CSH and is deserving of CSH recognition.
...
\begin{enumerate}
	...
	\item E-Board decides whether or not to present the nomination to CSH for a secret ballot Relative Majority vote.
	      If E-Board decides not to present the nomination to CSH, the selection process ends and the candidate does not become a member.
	...
	\item Candidates selected for Advisory Membership are notified of their acceptance as CSH Advisors and asked to accept or decline
	      the selection.
\end{enumerate}
...
Any large expenditures or large effect decisions must be brought before all of House.
