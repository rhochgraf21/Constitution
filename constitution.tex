...
\begin{tabular}{l l}
Technical Seminar & Seminar relating to computing or electronics, as approved by the Evals Director
\end{tabular}
...
\asection{Name}
The name of this Special Interest House is CSH.

\asection{Derivation of Authority}
CSH shall recognize that it receives its right to function as a Special Interest House from the Center for Campus Life and the Department of ResLife.
...
% ARTICLE 3 - CONSTITUTIONAL STRUCTURE
\article{Constitutional Structure}

\asection{Constitution}
The CSH Constitution is written and maintained by CSH and defines the major aspects, goals, and governing structure of CSH.

\asection{Standard Operating Session}
The Standard Operating Session for CSH is during the fall and spring semesters.
...
\begin{description}
	\item[Qualifications:] What criteria an applicant needs to be considered
	...
	\item[Leave of Absence:] The process by which a member may leave CSH for a period of time
	\item[Resignations:] The process by which a member terminates CSH membership
	\item[Term:] The duration of the membership type
\end{description}

\asection{Intro Membership}

\asubsection{Intro Membership Qualifications}
Intro Membership is open to all RIT students.

% remove and move children up, keep title TODO
\asubsection{Intro Membership Selection}
% Applicants notify the Evals Director of their interest in membership by submitting an application. TODO
...
The member does, however, have the right to use CSH's facilities.
...
\begin{enumerate}
	...
	\item The application materials are reviewed at an Evals meeting.
	      The applicant is accepted or rejected as an Intro member by a Relative Majority vote of attending members
\end{enumerate}
...
\begin{enumerate}
	...
	\item A subset of applicants will be offered Intro Membership and are given the opportunity to participate in the Intro Process defined in \ref{The Intro Process}.
...
\asubsection{Intro Membership Expectations}
Intro Members are expected to meet all the requirements of the Intro Process, as described in \ref{Expectations of an Intro Member}.

\asubsection{Intro Membership Privileges}
Intro Members receive the right to use CSH facilities, to attend CSH functions, and to have housing priority over all persons who are not CSH members.

% do this better
\asubsection{Intro Membership Evals}
%\asubsection{The Introductory Process}
The Intro Process is designed to provide an easy means for Intro Members to meet current members, learn CSH history, and be evaluated for Active Membership by current Active Members.

% fix numbers to be only text
\asubsubsection{The Eval Period}
The Intro Process will occur either once or twice per semester, and will last for six weeks.
The Intro Process shall be initiated by the Evals Director during either the first or second week of each semester.
E-Board may hold a closed ballot E-Board Vote to extend the Intro Process or the Intro Packet.
If an Intro Process is extended, the Intro Eval shall occur at the termination of the extended process, which may be outside of the period defined in \ref{Intro Eval}.
...
\asubsubsection{The Intro Packet}
Each Intro Process participant, after the first week, is given two weeks to obtain signatures from all Active Members who have passed a Membership Eval, all On-Floor Intro Members, and ten of any combination of Alumni, Honorary, and Advisory Members.
At the discretion of the Evals Director, any Intro Packet may be extended to accommodate extenuating circumstances.

\asubsubsection{Expectations of an Intro Member}
...
\begin{itemize}
	...
	\item Attend at least one directorship meeting for each week of the process (including permanent and Ad Hoc Directorship meetings)
...
\asubsubsection{Intro Eval}
The Intro Eval is the process by which memers choose which Intro Members to extend an offer of Active Membership.
It occurs at the end of the final week of the Intro Process.

\asubsubsubsection{Voting}
On a member by member basis, this eval process determines if each Intro Member has successfully completed the Intro Process requirements \ref{Expectations of an Intro Member}.
A Relative Majority Vote with a quorum of two-thirds of the number of Eligible Votes is required for the eval to be valid.
...
Each Intro Member may be given any of the possible outcomes listed in \ref{Outcomes}.

\asubsubsubsection{Outcomes}
\renewcommand{\theenumi}{\alph{enumi}} % For this section, we want items to use letters
\begin{enumerate}
	\item Intro Members may be offered Active Membership (\ref{Active Membership}) provided they meet the requirements described in \ref{Expectations of an Intro Member}, at the discretion of the attending members.
	...
	\item An Intro Member may be given a conditional to complete as a means of making up for missing requirements.
	      A conditional may be proposed by any member present at the Intro Eval and, if it is approved by the Evals Director, is then voted on by attending members.
...
\asubsection{Intro Membership Leave of Absence}
...
\asubsection{Intro Membership Resignations}
...
\asubsection{Intro Membership Term}
...
Active Membership is open to all students currently enrolled at RIT who have passed the Intro Evals and their most recent Membership Eval.
...
\\*\\*
Any Alumni Member in bad standing (\ref{Alumni Membership Selection}) may become an Active Member by notifying the Evals Director of their intent to participate.
...
Active Members are expected to be active participants in CSH as defined in \ref{Expectations of House Members}.
...
As an option to this requirement, members may instead assist on a large number of CSH activities and projects.
...
\begin{itemize}
	...
	\item Attend CSH functions
	\item Receive priority for available housing on-floor when returning from co-op
...
\begin{itemize}
	...
	\item Attend at least two technical seminars
\end{itemize}
\asubsection{Active Membership Evals}
Active Members are evaluated annually through the Eval Process as described in \ref{Evals Processes}.

% ACTIVE MEMBERSHIP EVALUATIONS

\asubsubsection{Evals Processes}
During the academic year, a CSH member is evaluated by the Membership Eval.
...
These meetings are open only to current Active Members.
All E-Board Members are expected to attend the these Evals Meetings to assist in the evaluations of CSH members.
\\* \\*
...
It is incumbent upon each member to provide the Evals Director with whatever information they feel is necessary to ensure an accurate eval.
\asubsubsubsection{Membership Eval}
The Membership Eval occurs once per academic year.
It is performed as part of the Eval Process that takes place during the spring semester to comply with RIT Housing deadlines.
\asubsubsubsubsection{Voting}
Any Active Member who has completed all of the requirements as defined in \ref{Expectations of House Members} at the beginning of the Membership Eval passes their Membership Eval without being voted on or evaluated by the quorum.
All Active Members who have not recieved an exemption from E-Board prior to the Membership Eval will be evaluated on a member by member basis by a quorum of Active Members.
The Membership Eval shall hold members to the objective requirements defined in \ref{Expectations of House Members} and determine which members may continue as an Active Member in the following academic year.
...
A Secret Immediate Relative Majority Vote with a quorum of two-thirds of the Eligible Votes is required for the eval to be valid.
Neither absentee nor proxy votes are allowed.

\asubsubsubsubsection{Outcomes}
Members who pass the Membership Eval have the option to participate as an Active Member in the following year and remain on the upcoming roster if applicable.
\\* \\*
If a member fails and has never passed a Membership Eval in the past, their membership will be revoked immediately.
If the member fails, but has previously passed a Membership Eval they will move to Alumni Membership status at the end of the Standard Operating Session (\ref{Alumni Membership Selection}).
...
If the conditional is approved by the Evals Director, it is then voted on by attending members.
...
If a member disagrees with the outcome of any eval, (e.g. is not asked to return to CSH for the following year) and wishes to appeal the decision, they may do so as stated in \ref{Appeals}.
\\* \\*
...
Active Members who depart house (i.e. resign) after passing the current operating session's Membership Eval are considered to be Alumni in good standing.
\\*\\*
...
Alumni Members shall receive the privelege to:
\begin{itemize}
	\item Use CSH facilities
	\item Attend CSH functions
\end{itemize}

\asubsection{Alumni Membership Evals}
...
\begin{enumerate}
	...
	\item E-Board decides whether or not to present the nomination at a House Meeting for a Secret Ballot Relative Majority Vote.
	      If E-Board decides not to present the nomination at a House Meeting, the selection process ends and the candidate does not become a member.
	\item The nomination is presented at a House Meeting for discussion and a Two-Thirds Vote
...
Honorary Members are encouraged to remain in contact with CSH.
\asubsection{Advisory Membership Expectations}
Advisors are encouraged to offer advice and assistance to CSH in any capacity the Advisor is able to help.
Advisors are also encouraged to meet CSH members and occasionally attend CSH activities.
...
Honorary and Advisory Members are not subject to formal evals.
...
A Leave of Absence offers the option for members to take extended time away from their responsibilities to CSH for any number of personal issues including, but not limited to, physical illness, mental illness, or care giving for a sick family member.
...
They are generally very task oriented and are chaired by a CSH Member.
...
Any large expenditures or large effect decisions must be brought before all Active Members at a House Meeting.
...
\begin{enumerate}
	...
	\item To meet, as an E-Board, at least weekly during the Standard Operating Session, as defined in \ref{Standard Operating Session} to discuss and report the operations of CSH
	...
	\item To make the final vote regarding conditionals and appeals as defined in \ref{Membership Eval}
...
\asubsubsection{Responsibilities of the Chair}
\begin{enumerate}
...
\asubsubsection{Responsibilities of the Evals Director}
\begin{enumerate}
...
\begin{enumerate}
	\item To preside over Social meetings, in which members are encouraged to bring ideas for social events
	\item To exercise general supervision over Social operations
	\item To oversee the organization, initiation, and execution of CSH social activities
	\item To ensure that there is a variety of events for members to participate in throughout the academic year
	\item To vote on issues presented to E-Board
\end{enumerate}

\asubsubsection{Responsibilities of the PR Director}
\begin{enumerate}
	\item To preside over PR meetings
	\item To operate CSH social media accounts intended to represent CSH as a whole
	\item To oversee the organization, initiation, and execution of CSH philanthropic events
...
\begin{enumerate}
	\item To preside over Financial meetings, in which members are encouraged to bring ideas for fundraising activities
...
\asubsubsection{Responsibilities of the \randd Director}
\begin{enumerate}
	\item To preside over R\&D meetings in which members are encouraged to bring ideas for projects
	...
	\item To provide seminars and tutorials to educate members in technical areas
	\item To vote on issues presented to E-Board
\end{enumerate}

\asubsubsection{Responsibilities of the IMPs Director}
\begin{enumerate}
...
\asubsubsection{Responsibilities of the OpComm Director}
\begin{enumerate}
	\item To represent the RTPs at E-Board Meetings and at House Meetings
	\item To report the status of the house systems, services, and network to E-Board and members
...
\begin{enumerate}
	...
	\item To oversee the creation of a yearbook outlining CSH events for the year
	\item To vote on issues presented to E-Board
\end{enumerate}
...
\asubsubsection{Qualifications to be the Chair, Evals Director}
\begin{enumerate}
...
\end{enumerate}
\asubsubsection{Qualifications to be the Social, Financial, \randd, IMPs, History, PR Director(s)}
\begin{enumerate}
	\item Candidates must be Active Members during the term of office
	\item Candidates must have at least one full semester of membership as an Active Member
...
\end{enumerate}
\asubsubsection{Qualifications to be the OpComm Director}
\begin{enumerate}
...
\asubsubsection{Qualifications to be the Secretary or an Ad Hoc Director}
\begin{enumerate}
...
\begin{enumerate}
	\item CSH may choose to waive the following subset of the qualifications for E-Board positions (defined under \ref{Qualifications}) for all candidates:
	      \begin{enumerate}
		      \item Candidates must reside on floor during the term of office.
		      \item Candidates must have at least one full semester of membership as an Active Member.
...
If they are also nominated as a normal directorship or as a dual directorship with another member, their votes are not cumulative.
...
\asubsubsection{Selection of the Chair, Evals Director, Social Director(s), Financial Director, IMPs Director, History Director,  \randd\ Director(s), PR Director}
\begin{enumerate}
...
\begin{enumerate}
	\item When E-Board or a group of members feels an Ad Hoc Directorship is necessary, they present their plans to E-Board.
...
E-Board may choose to select a current voting E-Board member, or to select any interested Active Member, as Secretary.
...
To postpone such a selection, the Chair may chair an Immediate Relative Majority Vote \ref{Immediate Vote} \ref{Relative Majority} during a House Meeting prior to the beginning of the selection process to delay the selection process by a specified amount of time.

\asection{Appointment of an Interim Director}
The duties and responsibilities of a vacated office are assumed by the Chair or by an Active member that is appointed at the Chair's discretion until the new director takes office.
...
\begin{enumerate}
	\item Impeachment of any E-Board Member may be initiated by petition, in writing, consisting of a minimum number of signatures of current CSH members equaling or exceeding one-third the number of Eligible Votes as defined in \ref{Eligible Votes}.
	...
	\item An E-Board Vote is taken on the impeachment petition, with all voting members present except the accused member who must be absent and whose vote counts as an abstention, to determine if the allegations stated in the petition are legitimate grounds for impeachment.
...
\asection{RTPs}
The OpComm Directorship is responsible for overseeing the implementation of maintenance and upgrades to the CSH computer systems network.
It is a self-governing committee making decisions by a Relative Majority Vote.
Membership is composed of all RTPs.

\asubsection{Selection of an RTP}
\renewcommand{\theenumi}{\alph{enumi}} % For this section, we want items to use letters
\begin{enumerate}
	...
	\item If an E-Board Member is a candidate for the office in discussion, the member is absent and their vote is abstained.
	      An E-Board Vote, as described in \ref{E-Board Vote}, is taken to determine whether the nominations of the RTP(s) are accepted.
\end{enumerate}
...
The current RTPs may from time to time draft rules and regulations specifying the rights and privileges of Prior RTPs.
...
\begin{enumerate}
	...
	\item Obtain greater than or equal to 60\% (rounded up to the nearest whole person) of required signatures (excluding those of On-Floor Members who have not passed a Membership Eval) in the Intro Packet or successfully complete Intro Eval.
\end{enumerate}
...
All dues must be collected in full before the annual Membership Eval (\ref{Membership Evaluation}).
...
All CSH expenditures must be approved by both the Financial Director and the appropriate Director (from whose budget the funds will be drawn).
...
\\* \\*
If the above amounts are to be exceeded, the expenditure must be approved by an Immediate Relative Majority Vote at House Meeting before the funds may be appropriated, as defined in \ref{Relative Majority}.
...
A quorum of one-quarter the number of current Active Members is required for the meeting to take place and for any vote to be considered valid, it must be passed by a Two-Thirds Vote.
\\*\\*
...
\\* \\*
If an appropriate Directorship cannot be determined for an expenditure, it is to be brought up for approval at House Meeting as a miscellaneous expenditure and approved by a Relative Majority Vote, regardless of the amount.
...
The Housing Priority System is a means for determining the priority a member has in a housing issue the Assignment of Available Housing.
...
If either the Chair or Evals Director are deemed biased or unfit for a position on the Judicial Board, they will be replaced by another Voting E-Board Member by means of an E-Board Vote of the remaining members.
...
Alternatively, the change may be presented at a House Meeting for discussion followed by an Immediate Vote.
A quorum of fifty percent of Eligible Votes is required for passage.
...
The final proposal is presented the following week and ballots are distributed for a Balloted Voteas described in \ref{Balloted Vote}.
...
The Constitution may be overridden by an Immediate Vote as described in \ref{Immediate Vote}.
There must be a quorum of eighty-five percent of Eligible Votes for the vote to be official.
