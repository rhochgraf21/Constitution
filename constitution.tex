...
The Constitution may be overridden by an Nine-Tenths Vote with eighty-five percent quorum as described in \ref{Immediate Vote}.
...
Intro Membership is open to all students currently enrolled in RIT.
...
A Voting E-Board Member is considered to be any E-Board Member with a non-zero vote on E-Board. % Defined this way so that (for example) each member in a dual directorship is each considered to be a voting member on E-Board.
...
Any large expenditures or large effect decisions must be brought up for discussion at a House Meeting.
...
\asubsubsection{Qualifications for Chair, Evals, Social, Financial, \randd, IMPs, History, PR Director(s)}
\begin{enumerate}
...
\begin{enumerate}
	...
	\item An RTP cannot be the director if they currently hold any other voting E-Board position or the position of Chair.
\end{enumerate}
...
If they are also nominated as a normal directorship or as dual directors with another member, their votes are not cumulative.
Each different nomination must be a separate entry on the election ballot.
Non-voting E-Board positions (i.e. Ad Hoc Directors) are not restricted to single or dual directors.
...
\begin{enumerate}
	...
	\item During an official E-Board vote, each dual director's vote counts for one-half vote in the tallying of votes.
